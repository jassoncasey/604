...
tryEval :: String -> String
tryEval "" = ""
tryEval s = 
  case parse "-" $ addSemiIfNot s of
  ErrPrg _ -> "Malformed expression. FIXME Better error?\n"
  parseProg -> case Ast.transformProg parseProg of
    Ast.ErrProg -> "Enable to transform parse tree to AST.\n"
    _ -> "Needs Eval!\n"
...
replSpl state = do
  ...
  if isCmd input
    ...
    else putStr (tryEval input) >> replSpl state
